...
import pandas as pd
from models import Session, Researcher, Relationship, User, AuditLog, Suggestion, ChronogramEntry, engine
...
# --- SAPP COMMITTEE ---
SAPP_COMMITTEE = [
    "Ignacio Escapa (MEF)", "Paula Narváez (IANIGLA)", "Marisol Beltrán", 
    "Josefina Bodnar", "Daniela Anabel Calderón", "Sonia Clara Camina",
    "Ari Iglesias", "Leandro Martínez", "Mauro Passalía", "Ezequiel Vera",
    "Bárbara Cariglino", "Silvia Gnaedinger", "Georgina Erra"
]

# --- CHRONOGRAM CONSTANTS ---
CHRONO_DAYS = DAYS = [
    'Domingo 6', 'Lunes 7', 'Martes 8', 'Miércoles 9', 'Jueves 10',
    'Viernes 11', 'Sábado 12', 'Domingo 13', 'Lunes 14'
]

CHRONO_TYPES = {
    'admin': {'label': 'Administración', 'color': '#F1F5F9', 'text': '#334155'},
    'keynote': {'label': 'Keynote', 'color': '#EEF2FF', 'text': '#4338CA'},
    'talk': {'label': 'Charla / Sesión', 'color': '#ECFDF5', 'text': '#047857'},
    'break': {'label': 'Break', 'color': '#FFFBEB', 'text': '#B45309'},
    'social': {'label': 'Social', 'color': '#FFF1F2', 'text': '#BE123C'},
    'poster': {'label': 'Poster', 'color': '#FAF5FF', 'text': '#7E22CE'},
    'field': {'label': 'Campo', 'color': '#F0F9FF', 'text': '#0369A1'},
    'course': {'label': 'Curso', 'color': '#FFF7ED', 'text': '#C2410C'},
    'outreach': {'label': 'Extensión', 'color': '#F0FDFA', 'text': '#0F766E'},
    'opening': {'label': 'Apertura / Cierre', 'color': '#F5F3FF', 'text': '#6D28D9'}
}

# Mapping React-style class combinations to simple HSL/Hex for Streamlit
TYPE_COLORS_ST = {
    'course': 'background-color: #eff6ff; color: #1e40af; border-color: #bfdbfe;',
    'talk': 'background-color: #f0fdf4; color: #166534; border-color: #bbf7d0;',
    'keynote': 'background-color: #fef2f2; color: #991b1b; border-color: #fecaca;',
    'break': 'background-color: #fffbeb; color: #92400e; border-color: #fef3c7;',
    'visit': 'background-color: #f5f3ff; color: #5b21b6; border-color: #ddd6fe;',
    'admin': 'background-color: #f8fafc; color: #64748b; border-color: #f1f5f9;',
    'field': 'background-color: #f0f9ff; color: #0284c7; border-color: #e0f2fe;',
    'course': 'background-color: #fff7ed; color: #ea580c; border-color: #ffedd5;',
    'outreach': 'background-color: #f0fdfa; color: #0d9488; border-color: #ccfbf1;',
    'opening': 'background-color: #faf5ff; color: #7c3aed; border-color: #f3e8ff;',
    'default': 'background-color: #f9fafb; color: #4b5563; border-color: #f3f4f6;'
}

INITIAL_CHRONO_DATA = {
    'Domingo 6': [
        {'id': str(uuid.uuid4()), 'start': '08:00', 'end': '12:00', 'label': 'Graduate Courses', 'type': 'course', 'room': 'G', 'location': 'UNPSJB'},
        {'id': str(uuid.uuid4()), 'start': '13:30', 'end': '17:30', 'label': 'Graduate Courses', 'type': 'course', 'room': 'G', 'location': 'UNPSJB'},
    ],
    'Lunes 7': [
        {'id': str(uuid.uuid4()), 'start': '08:00', 'end': '12:00', 'label': 'Graduate Courses', 'type': 'course', 'room': 'G', 'location': 'UNPSJB'},
        {'id': str(uuid.uuid4()), 'start': '13:30', 'end': '17:30', 'label': 'Graduate Courses', 'type': 'course', 'room': 'G', 'location': 'UNPSJB'},
        {'id': str(uuid.uuid4()), 'start': '09:30', 'end': '11:30', 'label': 'Visita Escuela Grupo 1', 'type': 'outreach', 'room': '1', 'location': 'Escuela A'},
        {'id': str(uuid.uuid4()), 'start': '09:30', 'end': '11:30', 'label': 'Visita Escuela Grupo 2', 'type': 'outreach', 'room': '2', 'location': 'Escuela B'},
        {'id': str(uuid.uuid4()), 'start': '09:30', 'end': '11:30', 'label': 'Visita Escuela Grupo 3', 'type': 'outreach', 'room': '3', 'location': 'Escuela C'},
        {'id': str(uuid.uuid4()), 'start': '09:30', 'end': '11:30', 'label': 'Visita Escuela Grupo 4', 'type': 'outreach', 'room': '4', 'location': 'Escuela D'},
        {'id': str(uuid.uuid4()), 'start': '09:30', 'end': '11:30', 'label': 'Visita Escuela Grupo 5', 'type': 'outreach', 'room': '5', 'location': 'Escuela E'},
        {'id': str(uuid.uuid4()), 'start': '14:00', 'end': '16:00', 'label': 'Visita Escuela Grupo 1', 'type': 'outreach', 'room': '1', 'location': 'Escuela A'},
        {'id': str(uuid.uuid4()), 'start': '14:00', 'end': '16:00', 'label': 'Visita Escuela Grupo 2', 'type': 'outreach', 'room': '2', 'location': 'Escuela B'},
        {'id': str(uuid.uuid4()), 'start': '14:00', 'end': '16:00', 'label': 'Visita Escuela Grupo 3', 'type': 'outreach', 'room': '3', 'location': 'Escuela C'},
        {'id': str(uuid.uuid4()), 'start': '14:00', 'end': '16:00', 'label': 'Visita Escuela Grupo 4', 'type': 'outreach', 'room': '4', 'location': 'Escuela D'},
        {'id': str(uuid.uuid4()), 'start': '14:00', 'end': '16:00', 'label': 'Visita Escuela Grupo 5', 'type': 'outreach', 'room': '5', 'location': 'Escuela E'},
    ],
    'Martes 8': [
        {'id': str(uuid.uuid4()), 'start': '10:20', 'end': '12:40', 'label': 'Pre-meeting Field Trip / Short Course / Registration', 'type': 'admin', 'location': 'Lobby TCC'},
        {'id': str(uuid.uuid4()), 'start': '12:40', 'end': '14:00', 'label': 'Pre-meeting Lunch', 'type': 'social', 'location': 'Lobby TCC'},
        {'id': str(uuid.uuid4()), 'start': '16:00', 'end': '18:20', 'label': 'Opening and Conference', 'type': 'opening', 'location': 'Sala A'},
        {'id': str(uuid.uuid4()), 'start': '18:20', 'end': '19:20', 'label': 'Ice Breaker', 'type': 'social', 'location': 'Lobby TCC'},
    ],
    'Miércoles 9': [
        {'id': str(uuid.uuid4()), 'start': '08:00', 'end': '08:20', 'label': 'Registration / Load PPT', 'type': 'admin', 'room': 'A', 'location': 'Lobby TCC'},
        {'id': str(uuid.uuid4()), 'start': '08:00', 'end': '08:20', 'label': 'Early Regular Session', 'type': 'talk', 'room': 'B', 'location': 'Sala B', 'children': [{'id': str(uuid.uuid4()), 'start': '08:00', 'end': '08:20', 'label': 'Regular Talk 1'}]},
        {'id': str(uuid.uuid4()), 'start': '08:20', 'end': '09:20', 'label': 'Keynote (Sala A)', 'type': 'keynote', 'location': 'Sala A'},
        {'id': str(uuid.uuid4()), 'start': '09:20', 'end': '10:20', 'label': 'Thematic Session – Sala A', 'type': 'talk', 'room': 'A', 'location': 'Sala A', 'children': []},
        {'id': str(uuid.uuid4()), 'start': '09:20', 'end': '10:20', 'label': 'Regular Session – Sala B', 'type': 'talk', 'room': 'B', 'location': 'Sala B', 'children': [
            {'id': str(uuid.uuid4()), 'start': '09:20', 'end': '09:40', 'label': 'Regular Talk 1'},
            {'id': str(uuid.uuid4()), 'start': '09:40', 'end': '10:00', 'label': 'Regular Talk 2'},
            {'id': str(uuid.uuid4()), 'start': '10:00', 'end': '10:20', 'label': 'Regular Talk 3'},
        ]},
        {'id': str(uuid.uuid4()), 'start': '10:20', 'end': '10:40', 'label': 'Coffee Break', 'type': 'break', 'location': 'Lobby TCC'},
        {'id': str(uuid.uuid4()), 'start': '12:40', 'end': '14:00', 'label': 'General Lunch', 'type': 'social', 'location': 'TCC Dining'},
        {'id': str(uuid.uuid4()), 'start': '14:00', 'end': '15:00', 'label': 'Keynote (Sala A)', 'type': 'keynote', 'location': 'Sala A'},
        {'id': str(uuid.uuid4()), 'start': '15:40', 'end': '16:00', 'label': 'Coffee Break', 'type': 'break', 'location': 'Lobby TCC'},
    ],
    'Jueves 10': [
        {'id': str(uuid.uuid4()), 'start': '08:00', 'end': '08:20', 'label': 'Registration / Load PPT', 'type': 'admin', 'location': 'Lobby TCC'},
        {'id': str(uuid.uuid4()), 'start': '08:20', 'end': '09:20', 'label': 'Keynote (Sala A)', 'type': 'keynote', 'location': 'Sala A'},
        {'id': str(uuid.uuid4()), 'start': '10:20', 'end': '10:40', 'label': 'Coffee Break', 'type': 'break', 'location': 'Lobby TCC'},
        {'id': str(uuid.uuid4()), 'start': '15:40', 'end': '16:00', 'label': 'Coffee Break / Group Photo', 'type': 'break', 'location': 'Lobby TCC'},
        {'id': str(uuid.uuid4()), 'start': '19:20', 'end': '23:00', 'label': 'Conference Dinner', 'type': 'social'},
    ],
    'Viernes 11': [
        {'id': str(uuid.uuid4()), 'start': '08:20', 'end': '09:20', 'label': 'Keynote (Sala A)', 'type': 'keynote', 'location': 'Sala A'},
        {'id': str(uuid.uuid4()), 'start': '10:20', 'end': '10:40', 'label': 'Coffee Break', 'type': 'break', 'location': 'Lobby TCC'},
        {'id': str(uuid.uuid4()), 'start': '17:20', 'end': '18:20', 'label': 'Closing Ceremony / Awards', 'type': 'opening', 'location': 'Sala A'},
    ],
    'Sábado 12': [
        {'id': str(uuid.uuid4()), 'start': '08:00', 'end': '18:00', 'label': 'Steppe, Monte and Whales (Full Day)', 'type': 'field', 'location': 'Península Valdés'},
        {'id': str(uuid.uuid4()), 'start': '08:00', 'end': '18:00', 'label': 'Gondwana Valley (Day 1 of 3)', 'type': 'field', 'location': 'Gondwana Valley'},
    ],
    'Domingo 13': [
        {'id': str(uuid.uuid4()), 'start': '08:00', 'end': '18:00', 'label': 'Gondwana Valley (Day 2 of 3)', 'type': 'field', 'location': 'Gondwana Valley'},
    ],
    'Lunes 14': [
        {'id': str(uuid.uuid4()), 'start': '08:00', 'end': '18:00', 'label': 'Gondwana Valley (Day 3 of 3)', 'type': 'field', 'location': 'Gondwana Valley'},
    ],
}
...
st.set_page_config(
    ...
    page_icon="🌿",
    initial_sidebar_state="expanded" if "edit_act" in st.query_params or (st.session_state.get('module') == 'Editor de Cronograma' and st.session_state.get('editing_item')) else "collapsed"
)
...
st.markdown("""
    ...
    /* Horizontal Scrolling Board for Chronogram Rooms (Selective) */
    .chrono-horizontal-board .stHorizontalBlock {
        flex-wrap: nowrap !important;
        overflow-x: auto !important;
        padding-bottom: 20px !important;
    }
    .chrono-horizontal-board .stHorizontalBlock > div {
        min-width: 320px !important;
        flex: 0 0 320px !important;
    }
    
    /* Custom Scrollbar */
    .chrono-horizontal-board .stHorizontalBlock::-webkit-scrollbar { height: 10px; }
    .chrono-horizontal-board .stHorizontalBlock::-webkit-scrollbar-track { background: #f8fafc; }
    .chrono-horizontal-board .stHorizontalBlock::-webkit-scrollbar-thumb { background-color: #cbd5e1; border-radius: 10px; border: 2px solid #f8fafc; }
    </style>
    """, unsafe_allow_html=True)

# --- SESSION STATE ---
# --- CHRONOGRAM DB SYNC ---
def load_chronogram_from_db():
    session = Session()
    entries = session.query(ChronogramEntry).all()
    session.close()
    
    if not entries:
        return INITIAL_CHRONO_DATA
    
    data = {}
    for entry in entries:
        encargados = json.loads(entry.encargados_json) if entry.encargados_json else []
        children = json.loads(entry.children_json) if entry.children_json else []
        ...
        data.setdefault(entry.day, []).append(item)
    return data

def save_chronogram_item(day, item):
    session = Session()
    db_item = session.query(ChronogramEntry).filter_by(id=item['id']).first()
    
    enc_json = json.dumps(item.get('encargados', []))
    child_json = json.dumps(item.get('children', []))
    
    if db_item:
        db_item.day = day
        db_item.start = item['start']
        db_item.end = item['end']
        db_item.label = item['label']
        db_item.type = item['type']
        db_item.room = item.get('room')
        db_item.location = item.get('location')
        db_item.encargados_json = enc_json
        db_item.children_json = child_json
    else:
        new_entry = ChronogramEntry(
            id=item['id'],
            day=day,
            start=item['start'],
            end=item['end'],
...
def delete_chronogram_item(item_id):
    session = Session()
    db_item = session.query(ChronogramEntry).filter_by(id=item_id).first()
    if db_item:
...
if 'chrono_data' not in st.session_state:
    st.session_state.chrono_data = load_chronogram_from_db()
if 'chrono_view' not in st.session_state:
    st.session_state.chrono_view = "weekly"
...
def module_selection_page():
    ...
    st.markdown("<h1 style='text-align: center; color: #2E5C8A;'>Centro de Gestión SAPP 2026</h1>", unsafe_allow_html=True)
    st.markdown("---")
    
    tab_mod, tab_circ, tab_links = st.tabs(["📦 Módulos", "📄 Circulares", "🔗 Links de Interés"])
    
    with tab_mod:
        col1, col2 = st.columns(2)
        with col1:
            st.markdown("""
            <div style='background: white; padding: 20px; border-radius: 12px; border: 1px solid #e0e0e0; box-shadow: 0 4px 6px rgba(0,0,0,0.05); text-align: center;'>
                <h3 style='color: #2E5C8A;'>🌿 Genealogía</h3>
                <p style='font-size: 0.9em; color: #666;'>Red de Paleobotánica y Palinología Argentina</p>
            </div>
            """, unsafe_allow_html=True)
            if st.button("Abrir Genealogía", use_container_width=True, key="btn_genealogy"):
                st.session_state.module = "genealogy"
                st.rerun()
                
        with col2:
            st.markdown("""
            <div style='background: white; padding: 20px; border-radius: 12px; border: 1px solid #e0e0e0; box-shadow: 0 4px 6px rgba(0,0,0,0.05); text-align: center;'>
                <h3 style='color: #E67E22;'>⏳ Cronograma</h3>
                <p style='font-size: 0.9em; color: #666;'>Editor de Cronograma del Congreso</p>
            </div>
            """, unsafe_allow_html=True)
            if st.button("Abrir Editor de Cronograma", use_container_width=True, key="btn_chronogram"):
                st.session_state.module = "chronogram"
                st.rerun()

    with tab_circ:
        st.subheader("📚 Circulares del Congreso")
        st.markdown("""
        Acceda a la información oficial distribuida a la comunidad científica:
        - 📄 **[Primera Circular](https://events.mef.org.ar/uploads/2025/08/1ra-circular-SAPP-AASP-Esp.pdf)**: Anuncio oficial y fechas clave.
        - 📄 **[Segunda Circular](https://events.mef.org.ar/uploads/2025/12/Segunda-circular.pdf)**: Detalles del comité y sede.

        """)

    with tab_links:
        st.subheader("🔗 Enlaces Importantes")
        st.info("Utilice estos enlaces para la gestión administrativa del congreso.")
        st.markdown(f"""
        - 📂 **[Documento Master del Congreso](https://docs.google.com/document/d/1szgb0LOb7QH9tt1vHNB3vjwC0atE6KijcCDnsMIWP0I/edit?usp=sharing)**: Coordinación interna.
        - 🌐 **[ALPP Official Website](https://www.alpp.online/)**: Asociación Latinoamericana de Paleobotánica y Palinología.
        - 🌐 **[AASP Official Website](https://palynology.org/)**: The Palynological Society.
        - 📸 **[Instagram ALPP](https://www.instagram.com/alpp_online/)**: Novedades y red social.
        - 📝 **[Planilla de Inscripción (SAPP 2026)](https://forms.gle/vPshj4D4gH3aV2fN7)**: Registro oficial de participantes.
        - 🌐 **[Sitio Web MEF SAPP 2026](https://events.mef.org.ar)**: Página oficial del evento.
...
def layout_column_activities(activities):
    sorted_acts = sorted(activities, key=lambda x: (get_minutes(x['start']), get_minutes(x['end'])))
    positioned = []
    active = []
    
    for act in sorted_acts:
        # Remove acts that end before this one starts
        active = [a for a in active if get_minutes(a['activity']['end']) > get_minutes(act['start'])]
        
        # Find first free lane
        used_lanes = set(a['lane'] for a in active)
        lane = 0
        while lane in used_lanes:
            lane += 1
            
        entry = {'activity': act, 'lane': lane}
        active.append(entry)
        positioned.append(entry)
        
    # Calculate lane counts for each cluster
    # This is a bit simplified compared to the React version but functional
    # We'll just use the max lane in the cluster as a proxy for width
    for entry in positioned:
        # Finding all overlapping items to decide width
        overlaps = [e for e in positioned if 
                   (get_minutes(e['activity']['start']) < get_minutes(entry['activity']['end']) and 
                    get_minutes(entry['activity']['start']) < get_minutes(e['activity']['end']))]
        entry['laneCount'] = max(e['lane'] for e in overlaps) + 1 if overlaps else 1
        
    return positioned

def get_room_label(room):
    if not room: return 'General / Plenario'
    if room == 'G': return 'Cursos Posgrado (UNPSJB)'
    if str(room).isdigit(): return f"Escuela Grupo {room}"
    return f"Sala {room}"

def render_timeline_activity_card(entry, day_start_min, pixel_scale=1.5, selected_id=None):
    act = entry['activity']
    lane = entry['lane']
    lane_count = entry['laneCount']
    
    style_base = TYPE_COLORS_ST.get(act['type'], TYPE_COLORS_ST['default'])
    
    # Calculate position
    act_start = get_minutes(act['start'])
    act_end = get_minutes(act['end'])
    top = (act_start - day_start_min) * pixel_scale
    height = max((act_end - act_start) * pixel_scale, 30) # Min height
    
    width_pct = 98 / lane_count
    left_pct = lane * width_pct
    
    # Highlight if selected
    border_style = "border: 2px solid #2563eb; box-shadow: 0 0 10px rgba(37,99,235,0.3);" if act['id'] == selected_id else ""
    
    children_html = ""
    if act.get('children'):
        children_html = "<div style='margin-top: 4px; display: flex; flex-wrap: wrap; gap: 2px;'>"
        for child in sorted(act['children'], key=lambda x: get_minutes(x['start'])):
            children_html += f"<div style='font-size: 0.6em; background: rgba(255,255,255,0.7); border: 1px solid rgba(0,0,0,0.1); border-radius: 3px; padding: 1px 4px; flex: 1 1 auto; min-width: 40px;'>{child['label']}</div>"
        children_html += "</div>"

    return f"<a href='?edit_act={act['id']}' target='_self' style='text-decoration: none; color: inherit;'><div style='{style_base} {border_style} position: absolute; top: {top}px; height: {height}px; width: {width_pct}%; left: {left_pct}%; padding: 8px; border-radius: 8px; border-left: 5px solid; overflow: hidden; transition: transform 0.1s; cursor: pointer;' onmouseover='this.style.transform=\"scale(1.02)\"; this.style.zIndex=\"10\";' onmouseout='this.style.transform=\"scale(1)\"; this.style.zIndex=\"1\";'><div style='font-size: 0.7em; font-weight: bold; opacity: 0.8;'>{act['start']} - {act['end']}</div><div style='font-weight: 800; font-size: 0.85em; text-transform: uppercase; line-height: 1; margin: 2px 0;'>{act['label']}</div>{children_html}</div></a>"


def chronogram_editor_app():
    # Handle deep-linking from cards
    qp = st.query_params
    if "edit_act" in qp:
        act_id = qp["edit_act"]
        # Find act across all days
        found = False
        for d in CHRONO_DAYS:
            for item in st.session_state.chrono_data.get(d, []):
                if item['id'] == act_id:
                    st.session_state.editing_item = item
                    st.session_state.selected_chrono_day = d
                    st.session_state.chrono_view = "detail"
                    st.query_params.clear() # Clear to avoid infinite loop
                    found = True
                    break
            if found: break
        if found: st.rerun()

    st.sidebar.markdown("### ⏳ Gestión de Cronograma")
    ...
    view = st.sidebar.radio("Vista", ["Vista Semanal", "Editor de Detalle"], 
                           index=0 if st.session_state.chrono_view == "weekly" else 1)
    st.session_state.chrono_view = "weekly" if view == "Vista Semanal" else "detail"

    if st.session_state.chrono_view == "weekly":
        render_chrono_weekly()
    else:
        render_chrono_detail()

def render_chrono_weekly():
    st.title("📅 Cronograma SAPP-AASP 2026")
    st.caption("Resumen semanal de actividades. Haga clic en un día para editar el detalle.")
    st.markdown("---")
    
    # Grid de 3 o 4 columnas dependiendo del ancho
    cols = st.columns(4) 
    for i, day in enumerate(CHRONO_DAYS):
        with cols[i % 4]:
            # Day Card Styling
            st.markdown(f"""
                <div style='background: #f8fafc; padding: 10px; border-radius: 10px; border: 1px solid #e2e8f0; margin-bottom: 10px;'>
                    <h3 style='margin:0; font-size: 1.1em; color: #1e293b;'>🗓️ {day}</h3>
                </div>
            """, unsafe_allow_html=True)
            
            items = st.session_state.chrono_data.get(day, [])
            if not items:
                st.info("Sin actividades.")
            else:
                for item in sorted(items, key=lambda x: get_minutes(x['start']))[:6]: # Mostrar hasta 6
                    style = TYPE_COLORS_ST.get(item['type'], TYPE_COLORS_ST['default'])
                    st.markdown(f"""<div style='{style} padding: 6px 10px; border-radius: 8px; border-left: 4px solid; margin-bottom: 6px; font-size: 0.85em; opacity: 1;'>
                        <span style='font-weight: bold; opacity: 0.8;'>{item['start']}</span> | <b>{item['label']}</b>
                    </div>""", unsafe_allow_html=True)
                if len(items) > 6:
                    st.markdown(f"<div style='font-size: 0.75em; color: #64748b; margin-bottom: 10px;'>... y {len(items)-6} más</div>", unsafe_allow_html=True)
            
            if st.button(f"Ver Detalle {day}", key=f"edit_day_{day}", use_container_width=True):
                st.session_state.selected_chrono_day = day
                st.session_state.chrono_view = "detail"
                st.rerun()

def render_chrono_detail():
    day = st.session_state.get('selected_chrono_day', CHRONO_DAYS[0])
    st.title(f"🛠️ Editor: {day}")
    
    if st.button("⬅️ Volver a vista semanal", key="btn_back_weekly"):
        st.session_state.chrono_view = "weekly"
        st.rerun()
        
    st.markdown("---")
    
    day_items = st.session_state.chrono_data.get(day, [])
    
    if not day_items:
        st.info("No hay actividades para este día.")
        if st.button("➕ Agregar primer bloque"):
            st.session_state.editing_item = {'id': str(uuid.uuid4()), 'label': '', 'start': '09:00', 'end': '10:00', 'type': 'talk', 'room': '', 'location': '', 'children': []}
    ...
    PIXEL_SCALE = 2.0
    day_start_min = min(get_minutes(i['start']) for i in day_items) - 30
    day_end_min = max(get_minutes(i['end']) for i in day_items) + 30
    timeline_height = (day_end_min - day_start_min) * PIXEL_SCALE

    # Identify unique rooms
    rooms = sorted(list(set(i.get('room') or '' for i in day_items)))
    
    selected_id = st.session_state.editing_item['id'] if st.session_state.get('editing_item') else None

    # Room Headers and Timeline Area - USING COLUMNS WITH CSS HACK
    st.markdown("<div class='chrono-horizontal-board'>", unsafe_allow_html=True)
    cols = st.columns(len(rooms))
    
    for ridx, room in enumerate(rooms):
        with cols[ridx]:
            # Room Header
            st.markdown(f"<div style='text-align: center; background: #f8fafc; padding: 12px; border-radius: 10px; border: 1px solid #e2e8f0; margin-bottom: 10px;'><h4 style='margin:0; font-size: 0.9em; color: #1e293b;'>🏛️ {get_room_label(room)}</h4></div>", unsafe_allow_html=True)

            room_items = [i for i in day_items if (i.get('room') or '') == room]
            positioned = layout_column_activities(room_items)
            cards_html = "".join(render_timeline_activity_card(entry, day_start_min, PIXEL_SCALE, selected_id) for entry in positioned)
            
            st.markdown(f"<div style='position: relative; height: {timeline_height}px; width: 100%; border-right: 1px dashed #e2e8f0;'>{cards_html}</div>", unsafe_allow_html=True)
            
            if st.button(f"➕ Bloque {room or ''}", key=f"add_btn_{room}", use_container_width=True):
    ...
    # Overlay Editor (Using sidebar as a "Drawer")
    if 'editing_item' in st.session_state:
        st.sidebar.markdown("---")
        st.sidebar.subheader("💎 Detalle de Actividad")
        item = st.session_state.editing_item
        
        # Quick View Info
        st.sidebar.info(f"""
        **Configuración Actual**:
        - 🏛️ {get_room_label(item.get('room'))}
        - 📍 {item.get('location') or 'Sin ubicación'}
        - 👥 {", ".join(item.get('encargados', [])) or 'Sin encargados asignados'}
        """)
        
        with st.sidebar.form("edit_form"):
            e_label = st.text_input("Título / Actividad", value=item['label'])
            e_type = st.selectbox("Tipo", list(CHRONO_TYPES.keys()), 
                                 index=list(CHRONO_TYPES.keys()).index(item['type']) if item['type'] in CHRONO_TYPES else 0,
                                 format_func=lambda x: CHRONO_TYPES[x]['label'])
            
            c3, c4 = st.columns(2)
            e_start = c3.text_input("Inicio", value=item['start'])
            e_end = c4.text_input("Fin", value=item['end'])
            
            e_room = st.text_input("Sala / Grupo", value=item.get('room') or '')
            e_loc = st.text_input("Ubicación específica", value=item.get('location') or '')
            
            # Encargados field (Responsible persons)
            e_resp = st.multiselect("Encargado/s (Comité SAPP)", 
                                   options=SAPP_COMMITTEE, 
                                   default=[r for r in item.get('encargados', []) if r in SAPP_COMMITTEE])
            e_resp_other = st.text_input("Otros encargados (separar por coma)", value=", ".join([str(r) for r in item.get('encargados', []) if r not in SAPP_COMMITTEE]))
            
            if st.form_submit_button("✅ Guardar Cambios"):
                final_resp = e_resp + [r.strip() for r in e_resp_other.split(",") if r.strip()]
                updated_item = {
                    **item,
                    'label': e_label,
                    'type': e_type,
                    ...
                    'room': e_room,
                    'location': e_loc,
                    'encargados': final_resp
                }
                
                if st.session_state.get('is_new_item'):
                    st.session_state.chrono_data.setdefault(day, []).append(updated_item)
                else:
                    st.session_state.chrono_data[day] = [
                        updated_item if i['id'] == item['id'] else i for i in day_items
                    ]
                
                # Save to DB
                save_chronogram_item(day, updated_item)
                
                del st.session_state.editing_item
                st.session_state.pop('is_new_item', None)
                st.rerun()
        
        # Tools for Splitting and Merging
        st.sidebar.markdown("### 🛠️ Herramientas de Bloque")
        col_t1, col_t2 = st.sidebar.columns(2)
        
        if col_t1.button("✂️ Dividir", help="Divide el bloque en dos partes iguales"):
            # Simple split logic
            m_s = get_minutes(item['start'])
            m_e = get_minutes(item['end'])
            m_mid = m_s + (m_e - m_s) // 2
            
            def min_to_str(m): return f"{m//60:02d}:{m%60:02d}"
            
            # Update current item
            item['end'] = min_to_str(m_mid)
            # Create new sibling
            new_item = {
                **item,
                'id': str(uuid.uuid4()),
                'start': min_to_str(m_mid),
                'end': min_to_str(m_e),
                'children': [] # Don't duplicate children for now
            }
            st.session_state.chrono_data[day].append(new_item)
            # Save both items to DB
            save_chronogram_item(day, item)
            save_chronogram_item(day, new_item)
            st.success("Bloque dividido")
            st.rerun()

        if col_t2.button("👯 Duplicar"):
        ...
        # Merge with next logic
        room_items = sorted([i for i in day_items if (i.get('room') or '') == (item.get('room') or '')], key=lambda x: get_minutes(x['start']))
        item_idx = next((i for i, x in enumerate(room_items) if x['id'] == item['id']), None)
        
        if item_idx is not None and item_idx < len(room_items) - 1:
            next_item = room_items[item_idx + 1]
            if st.sidebar.button(f"🔗 Fusionar con: {next_item['label'][:20]}...", use_container_width=True):
                # Extend current end to next end
                item['end'] = next_item['end']
                # Merge children
                item.setdefault('children', []).extend(next_item.get('children', []))
                # Delete next item from main data
                st.session_state.chrono_data[day] = [i for i in day_items if i['id'] != next_item['id']]
                
                # Save current and delete next from DB
                save_chronogram_item(day, item)
                delete_chronogram_item(next_item['id'])
                
                st.success("Bloques fusionados")
                st.rerun()
        ...
        # Sub-talks editor
        if not st.session_state.get('is_new_item'):
            st.sidebar.markdown("### 💬 Subcharlas")
            for cidx, child in enumerate(item.get('children', [])):
                with st.sidebar.expander(f"Talk: {child['label'] or 'Sin título'}"):
                    c_label = st.text_input("Título", value=child['label'], key=f"cl_{cidx}")
                    c_start = st.text_input("Inicio", value=child['start'], key=f"cs_{cidx}")
                    c_end = st.text_input("Fin", value=child['end'], key=f"ce_{cidx}")
                    if st.button("Eliminar Subcharla", key=f"cdel_{cidx}"):
                        item['children'].pop(cidx)
                        st.rerun()
                    # Apply changes to local item (will be saved on main form submit)
                    child['label'] = c_label
                    child['start'] = c_start
                    child['end'] = c_end
            
            if st.sidebar.button("➕ Agregar Subcharla"):
                item.setdefault('children', []).append({
                    'id': str(uuid.uuid4()),
                    'label': 'Nueva subcharla',
                    'start': item['start'],
                    'end': item['end']
                })
                st.rerun()
...
if __name__ == "__main__":
    if st.session_state.user:
        if st.session_state.module == "genealogy":
            main_app()
        elif st.session_state.module == "chronogram":
            chronogram_editor_app()
        else:
