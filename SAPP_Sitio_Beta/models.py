...
class ChronogramEntry(Base):
    __tablename__ = 'chronogram_entries'
    ...
    encargados_json = Column(Text) # JSON string
    children_json = Column(Text) # JSON string
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)

class ChronoGrid(Base):
    __tablename__ = 'chrono_grid'
    id = Column(Integer, primary_key=True)
    day = Column(String, nullable=False)
    timeslot = Column(String, nullable=False) # e.g. "08:00"
    room = Column(String, nullable=False) # e.g. "Sala A"
    activity_id = Column(String, ForeignKey('chronogram_entries.id')) # Link to full entry if exists
    content = Column(Text) # Fallback / direct text
    style_type = Column(String) # For simple coloring when not linked
